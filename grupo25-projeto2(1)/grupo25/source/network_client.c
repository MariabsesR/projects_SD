...
MessageT *network_send_receive(struct rtable_t *rtable, MessageT *msg)
{
    ...
    {
        // perror("Error allocating memory for serialized message");
        return NULL;
    }
    ...
    {
        free(serialized_msg);
        // perror("Error sending message size to the server");
        close(rtable->sockfd);
        return NULL;
    }
    ...
    {
        free(serialized_msg);
        // perror("Error sending message data to the server");
        close(rtable->sockfd);
        return NULL;
    }
    ...
    {
        // perror("Error reading response size from the server");
        close(rtable->sockfd);
        free(serialized_msg);
        return NULL;
    }
    ...
    {
        // perror("Error allocating memory for response buffer");
        close(rtable->sockfd);
        free(serialized_msg);
        return NULL;
    }
    ...
    {
        // perror("Error reading response data from the server");
        close(rtable->sockfd);
        free(serialized_msg);
